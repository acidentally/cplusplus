...
struct Trie {
	...
	int branch(node* l, node* r, int depth) {
		...
		//Tại nút không có rẽ nhánh, return ULLONG_MAX
		if(l == NULL || r == NULL) return INF; 

		if((l -> child[0] && r -> child[0]) ||
		   (l -> child[1] && r -> child[1])) { 
		   //<---- cả l r đều có chung bit thì ta lấy gtnn của hai hàm
		...
		return min(branch(l -> child[1], r -> child[0], depth - 1),
				   branch(l -> child[0], r -> child[1], depth - 1)) + (1 << depth); 
				   //<--- Không thì chịu :))
	}
